...
model Qmodel
  ...
  Modelica.Blocks.Sources.RealExpression Q1S[nEle](
    y=if InteractionBetweenPipes then -(U1 - U2)*len/nEle .* (Q_supply.T .- BC.T)
      else -(U1)*len/nEle .* (Q_supply.T .- BC.T))
    "Convective heat flow rate"
    annotation (Placement(transformation(extent={{-80,30},{-60,50}})));
  Modelica.Blocks.Sources.RealExpression Q1R[nEle](
    y=if InteractionBetweenPipes then -(U1 - U2)*len/nEle .* (Q_return.T .- BC.T)
      else -(U1)*len/nEle .* (Q_return.T .- BC.T))
    "Convective heat flow rate"
    annotation (Placement(transformation(extent={{-80,-50},{-60,-30}})));
  Modelica.Blocks.Sources.RealExpression Q1R2[nEle](
    y=if InteractionBetweenPipes then -(U2)*len/nEle .* (Q_supply.T .- Q_return.T)
      else zeros(nEle))
    "Convective heat flow rate"
    annotation (Placement(transformation(extent={{0,30},{20,50}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector[2] thermalCollector(
    each m=nEle)
    annotation (Placement(transformation(extent={{-10,10},{10,-10}},rotation=90,origin={-46,0})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow[nEle,2] AllHeat
    "Heat input into radiator from convective heat transfer"
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},rotation=180,origin={8,0})));
  ...
  Modelica.Fluid.Interfaces.HeatPorts_a[nEle] Q_supply
  ...
  Modelica.Fluid.Interfaces.HeatPorts_a[nEle] Q_return
    annotation (Placement(transformation(extent={{-10,-100},{10,-80}}),
      iconTransformation(extent={{-40,-100},{40,-80}})));
  Modelica.Blocks.Interfaces.RealInput[nPipes] U
...
<h4>Options</h4>
<p>
1- Different models for the <code>U1</code> and <code>U2</code> values can be choosen.
<a href=\"BuildingSystems.Technologies.DistrictHeatingNetworks.UndergroundPipes.BaseClasses.DHN_Umodels\"> U models Package</a>.
</p>
</html>"));
end Qmodel;
