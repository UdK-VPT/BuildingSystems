...
model ParallelPipes
  ...
  BuildingSystems.Fluid.Sources.MassFlowSource_T source(
    ...
    nPorts=1,
    m_flow=1,
    T=353.15) annotation (Placement(transformation(extent={{-40,10},{-20,30}})));
...
equation
  ...
  connect(parallelPipes.port_a, kusuda0D.port[1]) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  annotation (__Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Technologies/DistrictHeatingNetworks/UndergroundPipes/Examples/ParallelPipes.mos"
        "Simulate and plot"),Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{100,100}}), graphics));
end ParallelPipes;
