...
model Example
  ...
  EnergyProductionUnits.Central central(
    redeclare package Medium = Medium,
    m_flow_nominal=1,
    mProduction_flow_nominal=1,
    mDHN_flow_nominal=1,
  ...
  EnergyTransferStations.Station station1(
    ...
    show_T=true,
    factor_Tsupply=15,
    conPID(k=100, controllerType=Modelica.Blocks.Types.SimpleController.PI),
    Q_nominal=10000,
    Tsupply_max=353.15,
    Tsupply_min=333.15,
    Tset=294.15)
  ...
  Buildings.BuildingTemplates.Building1Zone0D                 building(
    AAmbient=2*(2*10+2*6.0)*2.8+10.0*6.0,
    AInner=10*10,
    AGround=10*10,
    nWindows=1,
    AWindow={2*3},
  ...
  Modelica.Blocks.Sources.Constant airchange(
    k=0.5)
    annotation (Placement(transformation(extent={{-6,6},{6,-6}},rotation=180,origin={-22,92})));
  ...
  Fluid.Storage.ExpansionVessel exp(
    redeclare package Medium = Medium,
    p_start=300000,
    V_start=1) annotation (Placement(transformation(extent={{10,60},{30,80}})));
...
equation
  connect(kusuda0D.port, thermalCollector.port_b) annotation (Line(
      points={{-201,-30},{-190,-30}},
      color={191,0,0},
      smooth=Smooth.None));
  ...
  connect(building.TAir[1], station.zoneTAir) annotation (Line(
      points={{-45,79},{-34,79},{-34,70},{-51,70},{-51,60}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(ambient.TAirRef, station.ambientTAirRef) annotation (Line(
      points={{-94.2,93},{-100,93},{-100,66},{-47,66},{-47,60}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(station.HeatPort, building.heatSourcesPorts[1]) annotation (Line(
      points={{-59,59},{-59,74},{-32,74},{-32,106},{-53.8,106},{-53.8,96}},
      color={191,0,0},
      smooth=Smooth.None));
  ...
  connect(building.airchange[1], airchange.y) annotation (Line(
      points={{-46.2,90},{-38,90},{-38,92},{-28.6,92}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(station2.port_b, parallelPipes1.port_a2) annotation (Line(
      ...
      color={0,127,255},
      smooth=Smooth.None));
  connect(thermalCollector.port_a[1], parallelPipes1.port_a) annotation (Line(
      points={{-169.333,-30},{-126,-30},{-126,48},{-102,48},{-102,39.8}},
      color={191,0,0},
      smooth=Smooth.None));
  connect(thermalCollector.port_a[2], parallelPipes.port_a) annotation (Line(
      points={{-170,-30},{-126,-30},{-126,10},{2,10},{2,46},{-20,46},{-20,39.8},
          {-18,39.8}},
      color={191,0,0},
      smooth=Smooth.None));
  connect(thermalCollector.port_a[3], parallelPipes2.port_a) annotation (Line(
      points={{-170.667,-30},{-126,-30},{-126,10},{14,10},{14,-50},{-10,-50},{
          -10,-60.2}},
      color={191,0,0},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-220,
    ...
    experiment(StopTime=300000, Interval=1800),
