...
model ConservationEquation "Lumped volume with mass and energy balance"
  ...
  output BuildingSystems.Interfaces.VolumeOutput fluidVolume_internal
    "Volume";
  input BuildingSystems.Interfaces.VolumeInput fluidVolume_in(
    ...
    annotation (Placement(transformation(extent={{-20,-20},{20,20}},rotation=0, origin={-120,-60}),
      iconTransformation(extent={{20,-20},{-20,20}},rotation=180,origin={-120,-60})));
...
equation
  ...
  if geometryType == BuildingSystems.Buildings.Types.GeometryType.Fixed then
    fluidVolume_internal = fluidVolume;
  else
    connect(fluidVolume_internal, fluidVolume_in);
  end if;
  ...
  if massDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then
    m = fluidVolume_internal*rho_start;
  else
    if _simplify_mWat_flow then
      ...
      m = fluidVolume_internal*Medium.density(Medium.setState_phX(
        p = medium.p,
        h = hOut,
        X = Medium.X_default));
    else
      // Use actual density
      m = fluidVolume_internal*medium.d;
    end if;
  end if;
  mXi = m*medium.Xi;
  if computeCSen then
...
The model
<a href=\"modelica://BuildingSystems.Fluid.MixingVolumes.Validation.MixingVolumeAdiabaticCooling\">
BuildingSystems.Fluid.MixingVolumes.Validation.MixingVolumeAdiabaticCooling</a>
...
Extension to a flexible fluid volume during run time.
</li>
<li>
...
This is for <a href=\"https://github.com/ibpsa/modelica-ibpsa/issues/1122\">BuildingSystems, issue 1122</a>.
</li>
<li>
April 16, 2018, by Michael Wetter:<br/>
Reformulated mass calculation so that Dymola can differentiate the equations.<br/>
This is for <a href=\"https://github.com/ibpsa/modelica-ibpsa/issues/910\">BuildingSystems, issue 910</a>.
</li>
<li>
