...
model PartialMixingVolume
  ...
  parameter Modelica.Units.SI.Volume V "Volume (if geometryType == Fixed)"
  ...
  input BuildingSystems.Interfaces.VolumeInput V_in(
    ...
    "Volume from input"
    annotation (Placement(transformation(extent={{-20,-20},{20,20}},rotation=0, origin={-120,-40}),
      iconTransformation(extent={{20,-20},{-20,20}},rotation=180,origin={-120,40})));
...
protected
  ...
  BuildingSystems.Buildings.Airvolumes.Interfaces.ConservationEquation dynBal(
...
");
  ...
  connect(heaFloSen.Q_flow, dynBal.Q_flow) annotation (Line(points={{-80,-10},{
          -80,-10},{-80,-16},{6,-16},{6,24},{50,24},{50,16},{58,16}},
                                                               color={0,0,127}));
  connect(V_in, dynBal.fluidVolume_in)
    annotation (Line(points={{-120,-40},{50,-40},{50,4},{58,4}}, color={0,0,127}));
