...
package Water "Package with model for liquid water with constant density"
  ...
  String(T_min-273.15) + " degC (" + String(T_min) + " Kelvin) as required from medium model \"BuildingSystems.Media.Water\".");

    assert(noEvent(T <= T_max), "In " + getInstanceName() + ": Temperature T = " + String(T) + " K exceeded its maximum allowed value of " +
  String(T_max-273.15) + " degC (" + String(T_max) + " Kelvin) as required from medium model \"BuildingSystems.Media.Water\".");

    assert(noEvent(p >= 0.0), "Pressure (= " + String(p) + " Pa) of medium \"BuildingSystems.Media.Water\" is negative\n(Temperature = " + String(T) + " K)");
...
<a href=\"modelica://BuildingSystems.Media.Specialized.Water.TemperatureDependentDensity\">
BuildingSystems.Media.Specialized.Water.TemperatureDependentDensity</a> which may have considerably higher computing time.
</p>
<p>
...
causes an error if
<a href=\"modelica://BuildingSystems.Media.Examples.WaterProperties\">
BuildingSystems.Media.Examples.WaterProperties</a>
...
<a href=\"https://github.com/ibpsa/modelica-ibpsa/blob/446aa83720884052476ad6d6d4f90a6a29bb8ec9/BuildingSystems/Media/Water.mo\">446aa83</a>.
</li>
<li>
