...
algorithm
  ...
  while abs(dv/v) > 1e-10 loop
    assert(m < 1E3,
      "Failed to converge in BuildingSystems.Media.Refrigerants.R410A.specificVolumeVap_pT");
    m := m + 1;

    // Evaluate first derivative of pressure w.r.t. specific volume
    dpdv := BuildingSystems.Media.Refrigerants.R410A.dPressureVap_dSpecificVolume_Tv(T, v);
    // Error on pressure
    dp := p - BuildingSystems.Media.Refrigerants.R410A.pressureVap_Tv(T, v);
...
annotation (derivative=BuildingSystems.Media.Refrigerants.R410A.dSpecificVolumeVap_pT,
