...
algorithm
  // Independent variable
  x := BuildingSystems.Utilities.Math.Functions.smoothMax(1-T/TCri, 1e-4, 5e-3) - x0;
  // Pressure of saturated refrigerant vapor
  p := pCri*Modelica.Math.exp(TCri/T*BuildingSystems.Utilities.Math.Functions.polynomial(a = a, x = x));
