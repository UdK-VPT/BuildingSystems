within BuildingSystems.Electrical.AC.ThreePhasesBalanced.Sensors.Examples;
model Probe "This example illustrates how to use the probe model"
  extends Modelica.Icons.Example;
  BuildingSystems.Electrical.AC.ThreePhasesBalanced.Loads.Capacitive loaRC(
    mode=BuildingSystems.Electrical.Types.Load.FixedZ_steady_state,
    P_nominal=-10000,
    V_nominal=480) "Constant load"
    annotation (Placement(transformation(extent={{10,0},{30,20}})));
  BuildingSystems.Electrical.AC.ThreePhasesBalanced.Sources.FixedVoltage sou(f=60, V=
        480) "Voltage source"
    annotation (Placement(transformation(extent={{-60,-30},{-40,-10}})));
  BuildingSystems.Electrical.AC.ThreePhasesBalanced.Lines.TwoPortResistance res1(R=0.1)
    "First line resistance"
    annotation (Placement(transformation(extent={{-26,0},{-6,20}})));
  BuildingSystems.Electrical.AC.ThreePhasesBalanced.Sensors.Probe probe_source(V_nominal=
       480) "Probe that measures at the voltage source"
    annotation (Placement(transformation(extent={{-50,30},{-30,50}})));
  BuildingSystems.Electrical.AC.ThreePhasesBalanced.Sensors.Probe probe_loadRC(V_nominal=
       480) "Probe that measures at the RC load"
    annotation (Placement(transformation(extent={{-10,30},{10,50}})));
  BuildingSystems.Electrical.AC.ThreePhasesBalanced.Lines.TwoPortResistance res2(R=0.1)
    "Second line resistance"
    annotation (Placement(transformation(extent={{-26,-70},{-6,-50}})));
  BuildingSystems.Electrical.AC.ThreePhasesBalanced.Loads.Inductive loaRL(
    mode=BuildingSystems.Electrical.Types.Load.FixedZ_steady_state,
    P_nominal=-10000,
    V_nominal=480) "Constant load"
    annotation (Placement(transformation(extent={{10,-70},{30,-50}})));
  BuildingSystems.Electrical.AC.ThreePhasesBalanced.Sensors.Probe probe_loadRL(V_nominal=
...
__Dymola_Commands(file=
          "modelica://BuildingSystems/Resources/Scripts/Dymola/Electrical/AC/ThreePhasesBalanced/Sensors/Examples/Probe.mos"
        "Simulate and plot"));
end Probe;
