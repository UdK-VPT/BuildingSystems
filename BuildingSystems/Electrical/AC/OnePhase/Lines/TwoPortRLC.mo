within BuildingSystems.Electrical.AC.OnePhase.Lines;
model TwoPortRLC "Model of an RLC element with two electrical ports"
  extends BuildingSystems.Electrical.Transmission.BaseClasses.PartialTwoPortRLC(
    V_nominal(start = 110),
  ...
  parameter Modelica.Units.SI.Voltage Vc_start[2]={V_nominal,0}
    ...
    annotation (Dialog(enable=(mode == BuildingSystems.Electrical.Types.Load.FixedZ_dynamic)));
  parameter BuildingSystems.Electrical.Types.Load mode(
    min=BuildingSystems.Electrical.Types.Load.FixedZ_steady_state,
    max=BuildingSystems.Electrical.Types.Load.FixedZ_dynamic)=
    BuildingSystems.Electrical.Types.Load.FixedZ_steady_state
...
initial equation
  if C > 0 and mode == BuildingSystems.Electrical.Types.Load.FixedZ_dynamic then
    Vc = Vc_start;
  end if;
equation
  ...
  L/2*omega*BuildingSystems.Electrical.PhaseSystems.OnePhase.j(terminal_p.i) +
    terminal_p.i*diagonal(ones(PhaseSystem_p.n)*R_actual/2) = terminal_p.v - Vc;
  L/2*omega*BuildingSystems.Electrical.PhaseSystems.OnePhase.j(terminal_n.i) +
    terminal_n.i*diagonal(ones(PhaseSystem_n.n)*R_actual/2) = terminal_n.v - Vc;

  if C > 0 then
    if mode == BuildingSystems.Electrical.Types.Load.FixedZ_dynamic then
      // Dynamics of the system
      C*der(Vc) + omega*C*BuildingSystems.Electrical.PhaseSystems.OnePhase.j(Vc) = Ic;
    else
      // steady state relationship
      omega*C*BuildingSystems.Electrical.PhaseSystems.OnePhase.j(Vc) = Ic;
    end if;
  else
...
<p align=\"center\">
<img alt=\"image\" src=\"modelica://BuildingSystems/Resources/Images/Electrical/AC/OnePhase/Lines/twoPortRLC.png\"/>
</p>
<p>
...
<a href=\"modelica://BuildingSystems.Electrical.AC.ThreePhasesBalanced.Lines.TwoPortRLC\">
BuildingSystems.Electrical.AC.ThreePhasesBalanced.Lines.TwoPortRLC</a>.
</li>
<li>
