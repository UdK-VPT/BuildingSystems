...
model Network "Single phase AC network"
  extends BuildingSystems.Electrical.Transmission.BaseClasses.PartialNetwork(
    V_nominal(start = 110),
  ...
  parameter BuildingSystems.Electrical.Types.Load modelMode=Types.Load.FixedZ_steady_state
    "Select between steady state and dynamic model" annotation (Dialog(
      tab="Model",
      group="Assumptions",
      enable=use_C), choices(choice=BuildingSystems.Electrical.Types.Load.FixedZ_steady_state
...
equation
  ...
  for i in 1:grid.nNodes loop
    VAbs[i] = BuildingSystems.Electrical.PhaseSystems.OnePhase.systemVoltage(terminal[i].v);
  end for;
