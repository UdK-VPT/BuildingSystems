...
model HeatAndMoistureTransport3D
  "Model for combinend 3D heat conduction and moisture transport"
  ...
  BuildingSystems.Interfaces.MoisturePort moisturePort_x2
     annotation(Placement(transformation(extent={{-8,-8},{8,8}},rotation=270,origin={80,20}),iconTransformation(extent={{-8,-8},{8,8}},rotation=270,origin={80,20})));
  BuildingSystems.Interfaces.MoisturePort moisturePort_y1
    annotation(Placement(transformation(extent={{-8,-8},{8,8}},rotation=270,origin={14,-80}),iconTransformation(extent={{-8,-8},{8,8}},rotation=270,origin={20,-80})));
  BuildingSystems.Interfaces.MoisturePort moisturePort_y2
    annotation(Placement(transformation(extent={{-8,-8},{8,8}},rotation=270,origin={14,80}), iconTransformation(extent={{-8,-8},{8,8}},rotation=270,origin={20,82})));
  BuildingSystems.Interfaces.MoisturePort moisturePort_z1
    annotation(Placement(transformation(extent={{-8,-8},{8,8}},rotation=270,origin={-68,-80}),iconTransformation(extent={{-8,-8},{8,8}},rotation=270,origin={-60,-80})));
  BuildingSystems.Interfaces.MoisturePort moisturePort_z2
    annotation(Placement(transformation(extent={{-8,-8},{8,8}},rotation=270,origin={80,70}), iconTransformation(extent={{-8,-8},{8,8}},rotation=270,origin={80,62})));
  ...
  BuildingSystems.Types.RelativeHumidity phi_y1
    "Relative humidity on the surface at side y1";
  BuildingSystems.Types.RelativeHumidity phi_y2
    "Relative humidity on the surface at side y2";
  BuildingSystems.Types.RelativeHumidity phi_z1
  ...
  Real Modelica.SIunits.MassFlowRate m_flow_wdr
    "Wind driven rain";
...
equation
  m_flow_wdr = 0.0; // wind driven rain
  heatPort_source.T = T;
  ...
  lengthY * lengthZ * lengthX*((C + w * cH2O)*der(T) + cH2O * T * der(w)) =
    CTh * heatPort_x1.T - 2.0 * CTh * T + CTh * heatPort_x2.T
    ...
    + CTh * heatPort_y1.T - 2.0 * CTh * T + CTh * heatPort_y2.T
    + Hv * (CMd * min(phi_y1,phiMax) * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_y1.T)
    - (CMd + CMd) * phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T)
    + CMd * min(phi_y2,phiMax) * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_y2.T))
    + CTh * heatPort_z1.T - 2.0 * CTh * T + CTh * heatPort_z2.T
    + Hv * (CMd * min(phi_z1,phiMax) * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_z1.T)
    - (CMd + CMd) * phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T)
    + CMd * min(phi_z2,phiMax) * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_z2.T))
    + heatPort_source.Q_flow;
  ...
  heatPort_y1.Q_flow =
    CTh * (heatPort_y1.T - T)
    + Hv * (CMd * (phi_y1 *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_y1.T) - phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T)));

  // Heat flux side y2
  heatPort_y2.Q_flow =
    CTh * (heatPort_y2.T - T)
    + Hv * (CMd * (phi_y2 *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_y2.T) - phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T)));

  // Heat flux side z1
  heatPort_z1.Q_flow =
    CTh * (heatPort_z1.T - T)
    + Hv * (CMd * (phi_z1 *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_z1.T) - phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T)));

  // Heat flux side z2
  heatPort_z2.Q_flow =
    CTh * (heatPort_z2.T - T)
    + Hv * (CMd * (phi_z2 *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_z2.T) - phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T)));
  ...
  lengthY * lengthZ * lengthX * der(w) =
    ...
    + CMd * min(phi_y1,phiMax) *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_y1.T)
    - (CMd + CMd) * phi *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T)
    + CMd * min(phi_y2,phiMax) *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_y2.T)
    + CLt * min(phi_y1,phiMax) - (CLt + CLt) * phi + CLt * min(phi_y2,phiMax)
    + CMd * min(phi_z1,phiMax) *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_z1.T)
    - (CMd + CMd) * phi *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T)
    + CMd * min(phi_z2,phiMax) *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_z2.T)
    + CLt * min(phi_z1,phiMax) - (CLt + CLt) * phi + CLt * min(phi_z2,phiMax);

  // Mass flow rate side x1
  ...
  // Mass flow rate side y1
  moisturePort_y1.m_flow =
    CMd*(phi_y1 *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_y1.T) - phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T))
    + CLt * (phi_y1 - phi);

  // Mass flow rate side y2
  moisturePort_y2.m_flow =
    CMd*(phi_y2 *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_y2.T) - phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T))
    + CLt * (phi_y2 - phi);

  // Mass flow rate side z1
  moisturePort_z1.m_flow =
    CMd*(phi_z1 *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_z1.T) - phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T))
    + CLt * (phi_z1 - phi);

  // Mass flow rate side z2
  moisturePort_z2.m_flow =
    CMd*(phi_z2 *  BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(heatPort_z2.T) - phi * BuildingSystems.Utilities.Psychrometrics.Functions.saturationPressure(T))
    + CLt * (phi_z2 - phi);

  // Relative humidities at the surfaces
  ...
  phi_y1 = BuildingSystems.Utilities.Psychrometrics.Functions.phi_pTX(100000.0,heatPort_y1.T,moisturePort_y1.x);
  phi_y2 = BuildingSystems.Utilities.Psychrometrics.Functions.phi_pTX(100000.0,heatPort_y2.T,moisturePort_y2.x);

  phi_z1 = BuildingSystems.Utilities.Psychrometrics.Functions.phi_pTX(100000.0,heatPort_z1.T,moisturePort_z1.x);
  phi_z2 = BuildingSystems.Utilities.Psychrometrics.Functions.phi_pTX(100000.0,heatPort_z2.T,moisturePort_z2.x);
  ...
  annotation(defaultComponentName = "heatConduction1DNodes",Icon(graphics={
    ...
    Text(extent={{-52,71},{16,5}},lineColor={120,0,120},lineThickness=0.5,fillColor={120,0,120},fillPattern=FillPattern.Solid,textString="3")}));
