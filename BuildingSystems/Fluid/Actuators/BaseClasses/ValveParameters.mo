...
partial model ValveParameters "Model with parameters for valves"
  ...
  parameter Modelica.Units.SI.Area Av(fixed=if CvData == BuildingSystems.Fluid.Types.CvTypes.Av
         then true else false) "Av (metric) flow coefficient" annotation (
      Dialog(group="Flow Coefficient", enable=(CvData == BuildingSystems.Fluid.Types.CvTypes.Av)));
