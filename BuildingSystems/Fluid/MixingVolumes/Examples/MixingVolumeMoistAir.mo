...
model MixingVolumeMoistAir
  ...
  Modelica.Blocks.Sources.Constant TSet(k=273.15 + 20)
    "Set point for temperature" annotation (Placement(transformation(extent={{
            -80,120},{-60,140}})));
  BuildingSystems.Utilities.Psychrometrics.pW_X humRat(           use_p_in=false)
    "Conversion from humidity ratio to partial water vapor pressure"
    annotation (Placement(transformation(extent={{-20,-120},{0,-100}})));
  BuildingSystems.Utilities.Psychrometrics.TDewPoi_pW dewPoi "Dew point temperature"
  ...
  BuildingSystems.Fluid.Sources.Boundary_pT sin(        redeclare package Medium =
    ...
    nPorts=1)             annotation (Placement(transformation(
        ...
        extent={{-10,-10},{10,10}},
        rotation=180)));
  BuildingSystems.Controls.Continuous.LimPID PI(
    Ni=0.1,
    yMax=1000,
    k=1,
    Ti=1,
  ...
  BuildingSystems.Controls.Continuous.LimPID PI1(
    Ni=0.1,
    Ti=1,
    controllerType=Modelica.Blocks.Types.SimpleController.PI,
    k=10,
  ...
  BuildingSystems.Fluid.Sensors.MassFlowRate mIn_flow(redeclare package Medium =
        Medium) annotation (Placement(transformation(extent={{6,-10},{26,10}})));
  BuildingSystems.Fluid.Sensors.MassFlowRate mOut_flow(redeclare package Medium =
...
experiment(StopTime=600,
           Tolerance=1e-6),
__Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Fluid/MixingVolumes/Examples/MixingVolumeMoistAir.mos"
        "Simulate and plot"),
