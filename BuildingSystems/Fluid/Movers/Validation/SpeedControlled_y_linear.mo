...
model SpeedControlled_y_linear
  ...
  BuildingSystems.Fluid.Movers.SpeedControlled_y pumFixDp(
    redeclare package Medium = Medium,
  ...
  BuildingSystems.Fluid.Sources.Boundary_pT sou1(
    ...
    nPorts=1) annotation (Placement(transformation(extent={{-10,-10},{10,10}},
          rotation=180,
        origin={128,90})));
  BuildingSystems.Fluid.FixedResistances.FixedResistanceDpM dp1(
    redeclare package Medium = Medium,
  ...
  BuildingSystems.Fluid.Movers.SpeedControlled_y pumFixM_flow(
    redeclare package Medium = Medium,
    energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState,
    dynamicBalance=false,
    per(pressure(V_flow=2/1000*{0, m_flow_nominal}, dp={2*dp_nominal, 0})),
...
__Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Fluid/Movers/Validation/SpeedControlled_y_linear.mos"
        "Simulate and plot"),
