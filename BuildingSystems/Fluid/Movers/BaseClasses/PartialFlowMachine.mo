...
partial model PartialFlowMachine
  ...
  replaceable parameter BuildingSystems.Fluid.Movers.Data.Generic per
    constrainedby BuildingSystems.Fluid.Movers.Data.Generic
    "Record with performance data"
    annotation (choicesAllMatching=true,
      Placement(transformation(extent={{52,60},{72,80}})));

  parameter BuildingSystems.Fluid.Types.InputType inputType = BuildingSystems.Fluid.Types.InputType.Continuous
    "Control input type"
    annotation(Dialog(
      group="Control"));
  parameter Real constInput = 0 "Constant input set point"
    annotation(Dialog(
      group="Control",
      enable=inputType == BuildingSystems.Fluid.Types.InputType.Constant));
  parameter Real stageInputs[:]
    "Vector of input set points corresponding to stages"
    annotation(Dialog(
      group="Control",
      enable=inputType == BuildingSystems.Fluid.Types.InputType.Stages));
  ...
  Modelica.Blocks.Interfaces.IntegerInput stage
    if inputType == BuildingSystems.Fluid.Types.InputType.Stages
...
protected
  ...
  parameter BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable preVar "Type of prescribed variable";
  ...
  Modelica.Blocks.Sources.Constant setConst(
    final k=constInput)
   if inputType == BuildingSystems.Fluid.Types.InputType.Constant
    "Constant input set point"
    annotation (Placement(transformation(extent={{-80,70},{-60,90}})));

  Extractor extractor(final nin=size(stageInputs,1))
   if inputType == BuildingSystems.Fluid.Types.InputType.Stages "Stage input extractor"
  ...
  BuildingSystems.Fluid.Delays.DelayFirstOrder vol(
  ...
  BuildingSystems.Fluid.Movers.BaseClasses.IdealSource preSou(
    ...
    final m_flow_small=m_flow_small,
    final allowFlowReversal=allowFlowReversal,
    final control_m_flow= (preVar ==  BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable.FlowRate))
    "Pressure source"
    annotation (Placement(transformation(extent={{40,-10},{60,10}})));

  BuildingSystems.Fluid.Movers.BaseClasses.PowerInterface heaDis(
 ...
 if addPowerToMedium
  ...
  BuildingSystems.Fluid.Sensors.RelativePressure senRelPre(
  ...
  // to zero.
  BuildingSystems.Fluid.Movers.BaseClasses.FlowMachineInterface eff(
    per(
...
Set nominalValuesDefineDefaultPressureCurve=true to suppress this warning.",
  ...
  assert(nominalValuesDefineDefaultPressureCurve or
         (per.havePressureCurve or
           (preVar == BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed)) or
         per.etaHydMet<>
...
<a href=\"Modelica://BuildingSystems.Fluid.Movers.Preconfigured.SpeedControlled_y\">
BuildingSystems.Fluid.Movers.Preconfigured.SpeedControlled_y</a>
extends from
...
<a href=\"modelica://BuildingSystems.Fluid.Movers.Examples.MoverContinuous\">
BuildingSystems.Fluid.Movers.Examples.MoverContinuous</a>.
</li>
<li>
