...
protected
  ...
  Modelica.Blocks.Sources.Constant[size(stageInputs, 1)] stageValues(
    final k=stageInputs) if
      inputType == BuildingSystems.Fluid.Types.InputType.Stages "Stage input values"
  ...
  Extractor extractor(final nin=size(stageInputs,1)) if
      inputType == BuildingSystems.Fluid.Types.InputType.Stages "Stage input extractor"
  ...
  Modelica.Blocks.Math.Gain gaiSpe(y(final unit="1")) if
    inputType == BuildingSystems.Fluid.Types.InputType.Continuous and
    speedIsInput
