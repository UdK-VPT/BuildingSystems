...
equation
  ...
  Q_flow = if homotopyInitialization then
    homotopy(actual=BuildingSystems.Utilities.Math.Functions.spliceFunction(pos=QThe_flow, neg=0,
                     x=noEvent(abs(VMachine_flow))-2*delta_V_flow, deltax=delta_V_flow),
                     simplified=0)
    else
      BuildingSystems.Utilities.Math.Functions.spliceFunction(pos=QThe_flow, neg=0,
                       x=noEvent(abs(VMachine_flow))-2*delta_V_flow, deltax=delta_V_flow);
