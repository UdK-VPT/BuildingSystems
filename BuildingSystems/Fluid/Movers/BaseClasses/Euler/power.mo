...
function power
  ...
  input BuildingSystems.Fluid.Movers.BaseClasses.Euler.peak peak "Peak operation point";
  input BuildingSystems.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal
    pressure "Pressure curve with both max flow rate and max pressure";
  output BuildingSystems.Fluid.Movers.BaseClasses.Euler.powerWithDerivative power(
...
algorithm
  ...
  for i in 2:10 loop
    power.P[i]:=V_flow[i] * dp[i]
                / BuildingSystems.Fluid.Movers.BaseClasses.Euler.efficiency(
                    peak=peak,
                    dp=dp[i],
                    V_flow=V_flow[i],
  ...
  power.d[2:10]:=BuildingSystems.Utilities.Math.Functions.splineDerivatives(
                   x=V_flow[2:10],
                   y=power.P[2:10],
                   ensureMonotonicity=false);
