within BuildingSystems.Fluid.Movers.BaseClasses;
model FlowMachineInterface
  ...
  parameter BuildingSystems.Fluid.Movers.Data.Generic per
    "Record with performance data"
    annotation (choicesAllMatching=true,
      Placement(transformation(extent={{60,-80},{80,-60}})));

  parameter BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable preVar=
    BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed "Type of prescribed variable";
...
protected
  final parameter Boolean preSpe=
    preVar == BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed
    "True if speed is a prescribed variable of this block";
  final parameter Boolean prePre=
    preVar == BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable.PressureDifference or
    preVar == BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable.FlowRate
  ...
  final parameter Real etaDer[size(per.efficiency.V_flow,1)]=
    if not per.etaHydMet==BuildingSystems.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate
    ...
    elseif (size(per.efficiency.V_flow, 1) == 1)
      ...
      BuildingSystems.Utilities.Math.Functions.splineDerivatives(
        ...
        ensureMonotonicity=BuildingSystems.Utilities.Math.Functions.isMonotonic(
  ...
  final parameter Real motDer[size(per.motorEfficiency.V_flow, 1)]=
    if not per.etaMotMet==BuildingSystems.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate
  ...
  final parameter Real motDer_yMot[size(per.motorEfficiency_yMot.y,1)]=
    if not per.etaMotMet==
      BuildingSystems.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio
  ...
  final parameter Real motDer_yMot_generic[9]=
    if per.etaMotMet==
      BuildingSystems.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve
      or  (per.etaMotMet==
      BuildingSystems.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.NotProvided
           and per.haveWMot_nominal)
      then BuildingSystems.Utilities.Math.Functions.splineDerivatives(
  ...
  final parameter
    BuildingSystems.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur1(
    final n = nOri,
  ...
  parameter
    BuildingSystems.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur2(
    final n = nOri + 1,
    ...
    "Volume flow rate vs. total pressure rise with correction for fan or pump's resistance added";
  parameter
    BuildingSystems.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur3(
    final n = nOri + 2,
    V_flow = if (haveVMax and haveDPMax) or (nOri == 2) then
               zeros(nOri + 2)
  ...
  parameter Real powDer[size(per.power.V_flow,1)]=
   if per.etaHydMet==
      BuildingSystems.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate then
     BuildingSystems.Utilities.Math.Functions.splineDerivatives(
                   x=per.power.V_flow,
                   y=per.power.P,
                   ensureMonotonicity=BuildingSystems.Utilities.Math.Functions.isMonotonic(x=per.power.P,
                                                                                     strict=false))
   else
     zeros(size(per.power.V_flow,1))
    "Coefficients for polynomial of power vs. flow rate";

  final parameter BuildingSystems.Fluid.Movers.BaseClasses.Euler.powerWithDerivative powEu_internal=
    if (curve == 1) then
      BuildingSystems.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur1)
    elseif (curve == 2) then
      BuildingSystems.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur2)
    else
      BuildingSystems.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur3)
    "Intermediate parameter";
  final parameter BuildingSystems.Fluid.Movers.BaseClasses.Characteristics.powerParameters powEu(
...
initial equation
  ...
  assert(BuildingSystems.Utilities.Math.Functions.isMonotonic(x=per.pressure.V_flow, strict=true) and
...
the simulation stops.");
  ...
  if curve == 1 then  // ----- Curve 1
    ...
    preDer1= BuildingSystems.Utilities.Math.Functions.splineDerivatives(x=pCur1.V_flow,
      y=pCur1.dp);
  ...
  elseif curve == 2 then  // ----- Curve 2
    ...
    preDer1= zeros(nOri);
    preDer2= BuildingSystems.Utilities.Math.Functions.splineDerivatives(x=pCur2.V_flow,
      y=pCur2.dp);
    preDer3= zeros(nOri + 2);
  else  // ----- Curve 3
    ...
    preDer1= zeros(nOri);
    preDer2= zeros(nOri + 1);
    preDer3= BuildingSystems.Utilities.Math.Functions.splineDerivatives(x=pCur3.V_flow,
      y=pCur3.dp);
  end if;
...
equation
  ...
  if (computePowerUsingSimilarityLaws == false) and preVar <> BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed then
    r_N=1;
  else
  ...
  WFlo = BuildingSystems.Utilities.Math.Functions.smoothMax(
  ...
  if per.powerOrEfficiencyIsHydraulic then
    ...
    etaHyd = BuildingSystems.Utilities.Math.Functions.smoothMin(
  ...
  if per.powerOrEfficiencyIsHydraulic then
    P_internal=WHyd;
    eta_internal=etaHyd;
    PEle = WFlo / BuildingSystems.Utilities.Math.Functions.smoothMax(
                    x1=eta, x2=1E-2, deltaX=1E-3);
  else
    P_internal=PEle;
    eta_internal=eta;
    WHyd = WFlo / BuildingSystems.Utilities.Math.Functions.smoothMax(
  ...
  if per.etaHydMet==
    ...
    eta_internal = WFlo/BuildingSystems.Utilities.Math.Functions.smoothMax(
                     x1=P_internal, x2=deltaP, deltaX=deltaP/2);
  elseif per.etaHydMet==
       BuildingSystems.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.EulerNumber then
    if homotopyInitialization then
    ...
    eta_internal = WFlo / BuildingSystems.Utilities.Math.Functions.smoothMax(
                            x1=P_internal, x2=deltaP, deltaX=deltaP/2);
  elseif per.etaHydMet == BuildingSystems.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
    ...
    if per.powerOrEfficiencyIsHydraulic then
      P_internal=WFlo/BuildingSystems.Utilities.Math.Functions.smoothMax(
                        x1=eta_internal, x2=1E-2, deltaX=1E-3);
    else
      P_internal=WHyd/BuildingSystems.Utilities.Math.Functions.smoothMax(
                        x1=eta_internal, x2=1E-2, deltaX=1E-3);
    end if;
  else // Not provided
    if per.powerOrEfficiencyIsHydraulic then
  ...
  if per.etaMotMet == BuildingSystems.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
  ...
  elseif per.etaMotMet==
       BuildingSystems.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio then
    if homotopyInitialization then
  ...
  elseif per.etaMotMet==
       BuildingSystems.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve then
      if homotopyInitialization then
        etaMot =homotopy(actual=cha.efficiency_yMot(
...
<a href=\"modelica://BuildingSystems.Fluid.Movers.Data.Generic\">
BuildingSystems.Fluid.Movers.Data.Generic</a>.
...
BuildingSystems.Fluid.Movers.UsersGuide</a>
...
<a href=\"modelica://BuildingSystems.Fluid.Movers.BaseClasses.PartialFlowMachine\">
BuildingSystems.Fluid.Movers.BaseClasses.PartialFlowMachine</a>.
</li>
<li>
Now it passes <code>WHyd</code> instead of <code>etaHyd</code> to
<a href=\"modelica://BuildingSystems.Fluid.Movers.BaseClasses.PowerInterface\">
BuildingSystems.Fluid.Movers.BaseClasses.PowerInterface</a>.
</li>
<li>
