...
model FlowControlled_m_flow
  ...
  extends BuildingSystems.Fluid.Movers.BaseClasses.PartialFlowMachine(
    final preVar=BuildingSystems.Fluid.Movers.BaseClasses.Types.PrescribedVariable.FlowRate,
  ...
  parameter Modelica.Units.SI.MassFlowRate constantMassFlowRate=m_flow_nominal
    "Constant pump mass flow rate, used when inputType=Constant" annotation (
      Dialog(enable=inputType == BuildingSystems.Fluid.Types.InputType.Constant));
  ...
  annotation (
      Icon(graphics={
        Text(
          ...
          textString=DynamicSelect("m_flow", if inputType == BuildingSystems.Fluid.Types.InputType.Continuous then String(m_flow_in, leftJustified=false, significantDigits=3) else String(stage)))}),
...
<li>October 1, 2009
    by Michael Wetter:<br/>
       Model added to the BuildingSystems library.
</ul>
</html>"));
