...
algorithm
  num := (0.387*BuildingSystems.Utilities.Math.Functions.smoothMax(Ra,1,0.1)^(1/6));
...
initial equation
  ...
  // Fluid properties
  mu_nominal = BuildingSystems.Fluid.HeatExchangers.BaseClasses.dynamicViscosityWater(
  ...
  rho_nominal = Medium.density(
        Medium.setState_pTX(
          ...
          T = 0.5*(TSur_nominal+TFlu_nominal),
          X = Medium.X_default));
  Pr_nominal = BuildingSystems.Fluid.HeatExchangers.BaseClasses.prandtlNumberWater(
          T = 0.5*(TSur_nominal+TFlu_nominal));

  B_nominal = BuildingSystems.Fluid.HeatExchangers.BaseClasses.isobaricExpansionCoefficientWater(
          T = 0.5*(TSur_nominal+TFlu_nominal));
...
equation
  ...
  state = Medium.setState_pTX(
             ...
             T = 0.5*(TSur+TFlu),
             X = Medium.X_default);
  mu = BuildingSystems.Fluid.HeatExchangers.BaseClasses.dynamicViscosityWater(
        T = 0.5 * (TSur+TFlu));
  rho = Medium.density(state);
  Pr = BuildingSystems.Fluid.HeatExchangers.BaseClasses.prandtlNumberWater(
          T = 0.5*(TSur+TFlu));

  B = BuildingSystems.Fluid.HeatExchangers.BaseClasses.isobaricExpansionCoefficientWater(
          T = 0.5*(TSur+TFlu));
  nu = mu/rho;
