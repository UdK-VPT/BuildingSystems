...
algorithm
  ...
  // effectiveness
  CMin_flow :=BuildingSystems.Utilities.Math.Functions.smoothMin(
    C1_flow,
    C2_flow,
    deltaCMin/4);
  CMax_flow :=BuildingSystems.Utilities.Math.Functions.smoothMax(
    C1_flow,
    C2_flow,
    deltaCMax/4);
  // CMin and CMax, constrained to be non-zero to compute eps-NTU-Z relationship
  CMinNZ_flow :=BuildingSystems.Utilities.Math.Functions.smoothMax(
    CMin_flow,
    deltaCMin,
    deltaCMin/4);
  CMaxNZ_flow :=BuildingSystems.Utilities.Math.Functions.smoothMax(
    CMax_flow,
    deltaCMax,
    deltaCMax/4);
  ...
  gaiEps := BuildingSystems.Utilities.Math.Functions.spliceFunction(
           pos=1,
           neg=0,
           x=CMin_flow-deltaCMin,
           deltax=deltaCMin/2);
  gaiNTU := BuildingSystems.Utilities.Math.Functions.spliceFunction(
           pos=1,
           neg=delta,
           x=CMin_flow-deltaCMin,
           deltax=deltaCMin/2);
  ...
  eps := gaiEps*BuildingSystems.Fluid.HeatExchangers.BaseClasses.epsilon_ntuZ(
                  NTU=NTU,
                  Z=Z,
...
<a href=\"modelica://BuildingSystems.Fluid.Types.HeatExchangerFlowRegime\">
BuildingSystems.Fluid.Types.HeatExchangerFlowRegime</a>.
</p>
</html>",
revisions="<html>
<ul>
...
<code>BuildingSystems.Fluid.Types.HeatExchangerFlowRegime</code>
to <code>Integer</code>.
This was done to have the same argument list as
<a href=\"modelica://BuildingSystems.Fluid.HeatExchangers.BaseClasses.epsilon_ntuZ\">
BuildingSystems.Fluid.HeatExchangers.BaseClasses.epsilon_ntuZ</a>,
