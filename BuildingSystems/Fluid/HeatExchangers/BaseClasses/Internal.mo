...
Received NTU = " + String(x) + "
         Z   = " + String(p));
    y := BuildingSystems.Fluid.HeatExchangers.BaseClasses.epsilon_ntuZ(
        NTU=x,
        Z=p,
        flowRegime=Integer(BuildingSystems.Fluid.Types.HeatExchangerFlowRegime.CrossFlowUnmixed));
  end f_nonlinear;
