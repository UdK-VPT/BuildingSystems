within BuildingSystems.Fluid.Storage.Validation;
model StratifiedNonUniformInitial
  ...
  BuildingSystems.Fluid.Sources.MassFlowSource_T sin_1(
    redeclare package Medium = Medium,
    T=273.15 + 20,
    m_flow=-m_flow_nominal,
    nPorts=1)
    "Mass flow source" annotation (Placement(transformation(extent={{80,10},{60,30}})));
  BuildingSystems.Fluid.Storage.Stratified heaTan(
    redeclare package Medium = Medium,
    hTan=3,
    dIns=0.3,
    VTan=0.1,
  ...
  BuildingSystems.Fluid.Storage.Stratified cooTan(
    redeclare package Medium = Medium,
    hTan=3,
    dIns=0.3,
    VTan=0.1,
  ...
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate HIn_flow(
    redeclare package Medium = Medium,
    m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
    annotation (Placement(transformation(extent={{-58,-58},{-42,-42}})));
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate HOut_flow(
    redeclare package Medium = Medium,
    m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
    annotation (Placement(transformation(extent={{2,-58},{18,-42}})));
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate HInEnh_flow(
    redeclare package Medium = Medium,
    m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
    annotation (Placement(transformation(extent={{-60,12},{-44,28}})));
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate HOutEnh_flow(
    redeclare package Medium = Medium,
    m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
...
annotation (experiment(Tolerance=1e-6, StopTime=3600),
  __Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Fluid/Storage/Validation/StratifiedNonUniformInitial.mos"
        "Simulate and plot"),
Documentation(info="<html>
This test model validates
<a href=\"modelica://BuildingSystems.Fluid.Storage.Stratified\">
BuildingSystems.Fluid.Storage.Stratified</a> by specifying a non-uniform initial 
temperature. 
</html>", revisions="<html>
<ul>
<li>
