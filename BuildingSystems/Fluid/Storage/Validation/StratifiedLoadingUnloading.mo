...
model StratifiedLoadingUnloading "Test model for stratified tank"
 ...
 parameter Modelica.SIunits.MassFlowRate m_flow_nominal= 1*1000/3600/4;
  ...
  BuildingSystems.Fluid.Storage.StratifiedEnhanced tanEnh(
    redeclare package Medium = Medium,
    hTan=3,
    dIns=0.3,
    VTan=0.1,
  ...
  BuildingSystems.Fluid.Storage.Stratified tan(
    redeclare package Medium = Medium,
    hTan=3,
    dIns=0.3,
    VTan=0.1,
  ...
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate HIn_flow(redeclare package Medium =
        Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
                                     annotation (Placement(transformation(
          extent={{-60,-38},{-44,-22}})));
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate HOut_flow(redeclare package Medium =
        Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
                                     annotation (Placement(transformation(
          extent={{22,-38},{38,-22}})));
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate HInEnh_flow(redeclare package Medium =
        Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
                                     annotation (Placement(transformation(
          extent={{-60,0},{-44,16}})));
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate HOutEnh_flow(redeclare package
...
equation
  ...
  connect(pulse.y, sin_2.m_flow_in) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  annotation (                     __Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Fluid/Storage/Validation/StratifiedLoadingUnloading.mos"
        "Simulate and plot"),
