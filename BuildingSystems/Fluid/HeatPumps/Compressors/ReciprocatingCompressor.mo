within BuildingSystems.Fluid.HeatPumps.Compressors;
...
equation
  ...
  pisDis_norm = BuildingSystems.Utilities.Math.Functions.smoothLimit(y, 0.0, 1.0, 0.001);

  if isOn then
    // Suction pressure
    pSuc = BuildingSystems.Utilities.Math.Functions.smoothMin(pEva - pDro, pCon - pDro, 0.01*ref.pCri);
    // Discharge pressure
    pDis = pCon + pDro;
    // Refrigerant mass flow rate
...
this is replaced by
<a href=\"modelica://BuildingSystems.Fluid.HeatPumps.Compressors.BaseClasses.TemperatureProtection\">
BuildingSystems.Fluid.HeatPumps.Compressors.BaseClasses.TemperatureProtection</a>.
