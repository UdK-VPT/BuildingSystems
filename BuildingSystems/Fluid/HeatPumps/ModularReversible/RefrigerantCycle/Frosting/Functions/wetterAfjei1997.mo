...
function wetterAfjei1997
  ...
  extends
    BuildingSystems.Fluid.HeatPumps.ModularReversible.RefrigerantCycle.Frosting.Functions.partialIcingFactor;
protected
...
algorithm
  ...
  gauTer :=gauFac*Modelica.Math.exp(-(TEvaInMea - gauMea)*(TEvaInMea - gauMea)/
    gauSig);
  fac := gauTer + BuildingSystems.Utilities.Math.Functions.smoothMax(
    x1=1E-5,
    x2= linTer,
    deltaX=0.25E-5);
  iceFac:=1 - fac;
