    ...
    for k in 1:J loop
      ...
      for n in 1:nPip loop
        ...
        for j in 1:J loop
          ...
          // Third term
          if m <> n then
            f := f + P_nj*BuildingSystems.Utilities.Math.Functions.binomial(j + k - 1, j -
          ...
          for jp in 0:j_pend loop
            f := f + sigma*Modelica.ComplexMath.conj(P_nj)*
              BuildingSystems.Utilities.Math.Functions.binomial(j, jp)*
              BuildingSystems.Utilities.Math.Functions.binomial(j + k - jp - 1, j - 1)*
