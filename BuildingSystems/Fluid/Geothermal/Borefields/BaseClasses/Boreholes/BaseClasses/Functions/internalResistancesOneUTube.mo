within BuildingSystems.Fluid.Geothermal.Borefields.BaseClasses.Boreholes.BaseClasses.Functions;
...
protected
  Real[2,2] RDelta(unit="(m.K)/W") "Delta-circuit thermal resistances";
  Real[2,2] R(unit="(m.K)/W") "Internal thermal resistances";
  ...
  Real[2] RFluPip(unit="(m.K)/W") = {RCondPipe+RConv, RCondPipe+RConv} "Fluid to pipe wall thermal resistances";
...
algorithm
  ...
  (RDelta, R) :=
    BuildingSystems.Fluid.Geothermal.Borefields.BaseClasses.Boreholes.BaseClasses.Functions.multipoleThermalResistances(
  ...
  BuildingSystems.Fluid.HeatExchangers.Boreholes.BaseClasses.singleUTubeResistances
  is
           hSeg = " + String(hSeg) + " m
           rBor = " + String(rBor) + " m
           rTub = " + String(rTub) + " m
