within BuildingSystems.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.LoadAggregation;
...
algorithm
  ...
  for j in 1:i loop
    if j==1 then
      ...
      prevT := BuildingSystems.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
        x=nu[j-1],
        x1=TStep[curInt,1],
        x2=TStep[curInt+1,1],
        y1=TStep[curInt,2],
    ...
    curT := BuildingSystems.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
      x=nu[j],
      x1=TStep[curInt,1],
      x2=TStep[curInt+1,1],
      y1=TStep[curInt,2],
