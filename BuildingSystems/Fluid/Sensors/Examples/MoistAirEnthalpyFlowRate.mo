...
model MoistAirEnthalpyFlowRate
  ...
  BuildingSystems.Fluid.Sensors.EnthalpyFlowRate senH_flow(
  ...
  BuildingSystems.Fluid.Sensors.SpecificEnthalpyTwoPort senH(
    redeclare package Medium = Medium,
    m_flow_nominal=1) "Specific enthalpy sensor"
                annotation (Placement(transformation(extent={{0,10},{20,30}})));
  BuildingSystems.Fluid.Sensors.MassFlowRate senM_flow(
    redeclare package Medium = Medium) "Mass flow rate sensor"
                annotation (Placement(transformation(extent={{28,10},{48,30}})));
  BuildingSystems.Utilities.Diagnostics.AssertEquality assEqu1
  ...
  BuildingSystems.Fluid.Sensors.LatentEnthalpyFlowRate senHLat_flow(
    redeclare package Medium = Medium,
    m_flow_nominal=1) "Latent enthalpy flow rate sensor"
    annotation (Placement(transformation(extent={{-60,-80},{-40,-60}})));
  BuildingSystems.Fluid.Sensors.SensibleEnthalpyFlowRate senHSen_flow(
  ...
  BuildingSystems.Utilities.Diagnostics.AssertEquality assEqu2
...
__Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Fluid/Sensors/Examples/MoistAirEnthalpyFlowRate.mos"
        "Simulate and plot"),
    Documentation(
    info="<html>
<p>
