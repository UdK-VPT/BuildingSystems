...
model PrescribedOutletState
  ...
  extends BuildingSystems.Fluid.Interfaces.PartialTwoPortTransport(
    final dp_start=0,
    show_T=false,
    show_V_flow=false);
...
equation
  ...
  if not restrictHeat and not restrictCool then
    ...
    m_flow_limited = BuildingSystems.Utilities.Math.Functions.smoothMax(
      x1=  port_a.m_flow,
      x2=  m_flow_small,
    ...
    if restrictHeat and restrictCool then
      ...
      dhSetAct = BuildingSystems.Utilities.Math.Functions.smoothLimit(
    ...
    elseif restrictHeat then
      ...
      dhSetAct = BuildingSystems.Utilities.Math.Functions.smoothMin(
        x1=hSet-inStream(port_a.h_outflow),
        x2=Q_flow_maxHeat / m_flow_limited,
        deltaX=deltah);
    else
      // Capacity limit for cooling only
      dhSetAct = BuildingSystems.Utilities.Math.Functions.smoothMax(
        x1=hSet-inStream(port_a.h_outflow),
...
See <a href=\"modelica://BuildingSystems.Fluid.HeatExchangers.HeaterCooler_T\">
BuildingSystems.Fluid.HeatExchangers.HeaterCooler_T</a>
