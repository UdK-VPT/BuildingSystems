...
package Medium = BuildingSystems.Media.Air;
  BuildingSystems.Utilities.Diagnostics.AssertEquality assTem(threShold=0.01)
    "Assert to test if the outputs of the forward flow and reverse flow model are identical"
    annotation (Placement(transformation(extent={{80,0},{100,20}})));
  BuildingSystems.Utilities.Diagnostics.AssertEquality assEnt(threShold=0.5)
    "Assert to test if the outputs of the forward flow and reverse flow model are identical"
    annotation (Placement(transformation(extent={{80,-30},{100,-10}})));
  BuildingSystems.Utilities.Diagnostics.AssertEquality assMas(threShold=1E-5)
    "Assert to test if the outputs of the forward flow and reverse flow model are identical"
    annotation (Placement(transformation(extent={{80,-60},{100,-40}})));
  BuildingSystems.Fluid.MassExchangers.Humidifier_u humBac(
  ...
  BuildingSystems.Fluid.MassExchangers.Humidifier_u humFor(
    redeclare package Medium = Medium,
    dp_nominal=0,
    m_flow(start=1),
    m_flow_nominal=1,
...
__Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Fluid/Interfaces/Examples/ReverseFlowHumidifier.mos"
        "Simulate and plot"),
