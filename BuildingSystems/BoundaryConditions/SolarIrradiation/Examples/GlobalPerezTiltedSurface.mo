within BuildingSystems.BoundaryConditions.SolarIrradiation.Examples;
model GlobalPerezTiltedSurface
  ...
  BuildingSystems.BoundaryConditions.SolarIrradiation.GlobalPerezTiltedSurface HGloRoo(
    til=BuildingSystems.Types.Tilt.Ceiling,
    azi=0.78539816339745,
    rho=rho)
    "Diffuse irradiation on roof"
    annotation (Placement(transformation(extent={{40,0},{60,20}})));
  BuildingSystems.BoundaryConditions.SolarIrradiation.GlobalPerezTiltedSurface HGloFlo(
    til=BuildingSystems.Types.Tilt.Floor,
    azi=0.78539816339745,
    rho=rho)
    "Diffuse irradiation on floor"
    annotation (Placement(transformation(extent={{40,-80},{60,-60}})));
  BuildingSystems.BoundaryConditions.SolarIrradiation.GlobalPerezTiltedSurface HGloWal(
...
equation
  ...
  __Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/BoundaryConditions/SolarIrradiation/Examples/GlobalPerezTiltedSurface.mos"
        "Simulate and plot"),
