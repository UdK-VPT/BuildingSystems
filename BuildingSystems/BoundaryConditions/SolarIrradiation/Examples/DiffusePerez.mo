...
model DiffusePerez
  ...
  BuildingSystems.BoundaryConditions.WeatherData.Bus weaBus annotation (Placement(
        transformation(extent={{1,-1},{21,21}}), iconTransformation(extent={{20,
            20},{21,21}})));
  BuildingSystems.BoundaryConditions.SolarIrradiation.DiffusePerez HDifRoo(
    til=BuildingSystems.Types.Tilt.Ceiling,
    lat=0.6457718232379,
    azi=0.78539816339745) "Diffuse irradiation on roof"
    annotation (Placement(transformation(extent={{40,0},{60,20}})));
  BuildingSystems.BoundaryConditions.SolarIrradiation.DiffusePerez HDifFlo(
    til=BuildingSystems.Types.Tilt.Floor,
    lat=0.6457718232379,
    azi=0.78539816339745) "Diffuse irradiation on floor"
    annotation (Placement(transformation(extent={{40,-80},{60,-60}})));
  BuildingSystems.BoundaryConditions.SolarIrradiation.DiffusePerez HDifWal(
...
__Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/BoundaryConditions/SolarIrradiation/Examples/DiffusePerez.mos"
        "Simulate and plot"),
