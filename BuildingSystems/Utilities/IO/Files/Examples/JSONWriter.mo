within BuildingSystems.Utilities.IO.Files.Examples;
model JSONWriter "Example use of the JSON writer"
  extends Modelica.Icons.Example;
  BuildingSystems.Utilities.IO.Files.JSONWriter jsonWriterInitial(
    nin=3,
    fileName="InitialOutputs.json",
    outputTime=BuildingSystems.Utilities.IO.Files.BaseClasses.OutputTime.Initial)
    "Outputs at initial time"
    annotation (Placement(transformation(extent={{0,20},{20,40}})));
  BuildingSystems.Utilities.IO.Files.JSONWriter jsonWriterCustom(
    nin=3,
    outputTime=BuildingSystems.Utilities.IO.Files.BaseClasses.OutputTime.Custom,
    customTime=0.5,
    fileName="CustomOutputs.json",
    varKeys={"Output1","Key2","Name3"}) "Outputs at custom time"
    annotation (Placement(transformation(extent={{0,-10},{20,10}})));
  BuildingSystems.Utilities.IO.Files.JSONWriter jsonWriterTerminal(
    nin=3,
...
See <a href=\"https://github.com/ibpsa/modelica-ibpsa/issues/1114\">#1114</a>.
    ...
    experiment(
      StopTime=2,
      Tolerance=1e-06),
    __Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Utilities/IO/Files/Examples/JSONWriter.mos"
        "Simulate and plot"));
end JSONWriter;
