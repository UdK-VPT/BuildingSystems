within BuildingSystems.Utilities.IO.Files;
model JSONWriter "Model for writing results to a json file"
  ...
  parameter BuildingSystems.Utilities.IO.Files.BaseClasses.OutputTime outputTime=
...
protected
  ...
  BuildingSystems.Utilities.IO.Files.BaseClasses.JSONWriterObject jsonWri=
      BuildingSystems.Utilities.IO.Files.BaseClasses.JSONWriterObject(
        insNam,
        fileName,
        outputTime==BuildingSystems.Utilities.IO.Files.BaseClasses.OutputTime.Terminal,
        varKeys)
    "File writer object";
equation
  if outputTime==BuildingSystems.Utilities.IO.Files.BaseClasses.OutputTime.Terminal then
    BuildingSystems.Utilities.IO.Files.BaseClasses.cacheVals(jsonWri, u);
  end if;

  if outputTime==BuildingSystems.Utilities.IO.Files.BaseClasses.OutputTime.Initial then
    when initial() then
      BuildingSystems.Utilities.IO.Files.BaseClasses.writeJSON(jsonWri, u);
    end when;
  end if;

  if outputTime==BuildingSystems.Utilities.IO.Files.BaseClasses.OutputTime.Custom then
