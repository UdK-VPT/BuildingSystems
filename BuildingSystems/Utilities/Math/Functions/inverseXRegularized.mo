...
algorithm
  y :=if (x > delta or x < -delta) then 1/x elseif (x < delta/2 and x > -delta/2) then x/(delta*delta) else
    BuildingSystems.Utilities.Math.Functions.BaseClasses.smoothTransition(
       x=x,
  ...
  derivative(order=1,
          ...
          zeroDerivative=d,
          zeroDerivative=e,
          zeroDerivative=f)=BuildingSystems.Utilities.Math.Functions.BaseClasses.der_inverseXRegularized,
              Inline=true,
...
<p>
See the plot of
<a href=\"modelica://BuildingSystems.Utilities.Math.Functions.Examples.InverseXRegularized\">
BuildingSystems.Utilities.Math.Functions.Examples.InverseXRegularized</a>
for the graph.
</p>
<p>
