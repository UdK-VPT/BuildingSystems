...
algorithm
  ...
  if k <= 0.5*n then
    binom := integer(BuildingSystems.Utilities.Math.Functions.fallingFactorial(n,k)/BuildingSystems.Utilities.Math.Functions.factorial(k));
  else
    binom := integer(BuildingSystems.Utilities.Math.Functions.fallingFactorial(n,n-k)/BuildingSystems.Utilities.Math.Functions.factorial(n-k));
  end if;
