...
model OneOpenDoor "Model with one open and one closed door"
  ...
  BuildingSystems.Airflow.Multizone.DoorDiscretizedOpen dooOpe(redeclare package Medium
  ...
  Modelica.Blocks.Math.Gain Gain1(k=100) annotation (Placement(transformation(
          extent={{-20,60},{0,80}})));
  BuildingSystems.Airflow.Multizone.DoorDiscretizedOperable dooOpeClo(redeclare package
...
equation
  ...
  connect(volB.ports[4], dooOpeClo.port_a2) annotation (Line(
      ...
      color={0,127,255}));
  annotation (
    __Dymola_Commands(file="modelica://BuildingSystems/Resources/Scripts/Dymola/Airflow/Multizone/Examples/OneOpenDoor.mos"
        "Simulate and plot"),
    experiment(
      StopTime=7200,
      Tolerance=1e-05),
