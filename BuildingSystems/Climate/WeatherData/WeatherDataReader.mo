...
block WeatherDataReader
  "Reader for TMY3 and NetCDF weather data"
  model WeatherDataNetCDF
    extends BuildingSystems.Climate.WeatherData.WeatherDataNetcdf;
    output BuildingSystems.Interfaces.RadiantEnergyFluenceRateOutput IrrTotHor = y[1] + y[2]
      "Solar total radiation of horizontal surface";
  end WeatherDataNetCDF;
  parameter Boolean NetCDF = true;
  BuildingSystems.BoundaryConditions.WeatherData.ReaderTMY3 weaDatTMY3(
    filNam = filNamTMY3,
    computeWetBulbTemperature = computeWetBulbTemperature,
    pAtmSou = pAtmSou,
    pAtm = pAtm,
    ceiHeiSou = ceiHeiSou,
    ceiHei = ceiHei,
    totSkyCovSou = totSkyCovSou,
    totSkyCov = totSkyCov,
    opaSkyCovSou = opaSkyCovSou,
    opaSkyCov = opaSkyCov,
    TDryBulSou = TDryBulSou,
    TDryBul = TDryBul,
    TDewPoiSou = TDewPoiSou,
    TDewPoi = TDewPoi,
    TBlaSkySou = TBlaSkySou,
    TBlaSky = TBlaSky,
    relHumSou = relHumSou,
    relHum = relHum,
    winSpeSou = winSpeSou,
    ...
    winDirSou = winDirSou,
    winDir = winDir,
    HInfHorSou = HInfHorSou,
    HInfHor = HInfHor,
    HSou) if not NetCDF
    annotation (Placement(transformation(extent={{-80,40},{78,200}})));
  WeatherDataNetCDF weaDatNetCDF(
    weatherDataFile = filNamNetCDF) if NetCDF
    annotation (Placement(transformation(extent={{-80,-200},{80,-40}})));
  ...
  Modelica.Blocks.Interfaces.RealInput pAtm_in(
    ...
    final unit="Pa",
    displayUnit="Pa") if (pAtmSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input pressure"
    annotation (Placement(transformation(extent={{-240,220},{-200,260}}),iconTransformation(extent={{-240,220},{-200,260}})));
  //--------------------------------------------------------------
  // Ceiling height
  parameter BuildingSystems.BoundaryConditions.Types.DataSource ceiHeiSou=BuildingSystems.BoundaryConditions.Types.DataSource.File
  ...
  Modelica.Blocks.Interfaces.RealInput ceiHei_in(
    final quantity="Height",
    final unit="m",
    displayUnit="m") if (ceiHeiSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input ceiling height"
    annotation (Placement(transformation(extent={{-240,-20},{-200,20}}),iconTransformation(extent={{-240,-20},{-200,20}})));
  //--------------------------------------------------------------
  // Total sky cover
  parameter BuildingSystems.BoundaryConditions.Types.DataSource totSkyCovSou=BuildingSystems.BoundaryConditions.Types.DataSource.File
    "Total sky cover"
    annotation (Evaluate=true, Dialog(group="Data source"));
  ...
  Modelica.Blocks.Interfaces.RealInput totSkyCov_in(
    min=0,
    max=1,
    unit="1") if (totSkyCovSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input total sky cover"
    annotation (Placement(transformation(extent={{-240,-60},{-200,-20}}),iconTransformation(extent={{-240,-60},{-200,-20}})));
  // Opaque sky cover
  parameter BuildingSystems.BoundaryConditions.Types.DataSource opaSkyCovSou=BuildingSystems.BoundaryConditions.Types.DataSource.File
    "Opaque sky cover"
    annotation (Evaluate=true, Dialog(group="Data source"));
  ...
  Modelica.Blocks.Interfaces.RealInput opaSkyCov_in(
    min=0,
    max=1,
    unit="1") if (opaSkyCovSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input opaque sky cover"
    annotation (Placement(transformation(extent={{-240,20},{-200,60}}),iconTransformation(extent={{-240,20},{-200,60}})));
  //--------------------------------------------------------------
  ...
  Modelica.Blocks.Interfaces.RealInput TDryBul_in(
    ...
    final unit="K",
    displayUnit="degC") if (TDryBulSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input dry bulb temperature"
    annotation (Placement(transformation(extent={{-240,140},{-200,180}}),iconTransformation(extent={{-240,140},{-200,180}})));
  //--------------------------------------------------------------
  ...
  Modelica.Blocks.Interfaces.RealInput TDewPoi_in(
    final quantity="ThermodynamicTemperature",
    final unit="K",
    displayUnit="degC") if (TDewPoiSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input dew point temperature"
    annotation (Placement(transformation(extent={{-240,180},{-200,220}}),iconTransformation(extent={{-240,180},{-200,220}})));
  //--------------------------------------------------------------
  ...
  Modelica.Blocks.Interfaces.RealInput TBlaSky_in(
    ...
    displayUnit="degC",
    final unit="K") if
    (TBlaSkySou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Black-body sky temperature"
    annotation (Placement(transformation(extent={{-240,100},{-200,140}}),iconTransformation(extent={{-240,100},{-200,140}})));
  //--------------------------------------------------------------
  // Relative humidity
  parameter BuildingSystems.BoundaryConditions.Types.DataSource relHumSou=BuildingSystems.BoundaryConditions.Types.DataSource.File
  ...
  Modelica.Blocks.Interfaces.RealInput relHum_in(
    min=0,
    max=1,
    unit="1") if (relHumSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input relative humidity"
    annotation (Placement(transformation(extent={{-240,60},{-200,100}}),iconTransformation(extent={{-240,60},{-200,100}})));
  //--------------------------------------------------------------
  // Wind speed
  ...
  Modelica.Blocks.Interfaces.RealInput winSpe_in(
    ...
    final unit="m/s",
    min=0) if (winSpeSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input wind speed"
    annotation (Placement(transformation(extent={{-240,-100},{-200,-60}}),iconTransformation(extent={{-240,-100},{-200,-60}})));
  //--------------------------------------------------------------
  // Wind direction
  ...
  Modelica.Blocks.Interfaces.RealInput winDir_in(
    ...
    final unit="rad",
    displayUnit="deg") if (winDirSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input wind direction"
  annotation (Placement(transformation(extent={{-240,-140},{-200,-100}}),iconTransformation(extent={{-240,-140},{-200,-100}})));
  //--------------------------------------------------------------
  ...
  Modelica.Blocks.Interfaces.RealInput HInfHor_in(
    ...
    final unit="W/m2") if (HInfHorSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input)
    "Input infrared horizontal radiation"
    annotation (Placement(transformation(extent={{-240,-220},{-200,-180}}),iconTransformation(extent={{-240,-220},{-200,-180}})));
  parameter BuildingSystems.BoundaryConditions.Types.RadiationDataSource HSou=BuildingSystems.BoundaryConditions.Types.RadiationDataSource.File
  ...
  Modelica.Blocks.Interfaces.RealInput HGloHor_in(
    final quantity="RadiantEnergyFluenceRate",
    final unit="W/m2") if (HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HGloHor_HDifHor or HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HGloHor)
    "Input global horizontal radiation"
    annotation (Placement(transformation(extent={{-240,-300},{-200,-260}}),iconTransformation(extent={{-240,-300},{-200,-260}})));
  //--------------------------------------------------------------
  // Diffuse horizontal radiation
  Modelica.Blocks.Interfaces.RealInput HDifHor_in(
    final quantity="RadiantEnergyFluenceRate",
    final unit="W/m2") if (HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HGloHor_HDifHor or HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HDifHor)
    "Input diffuse horizontal radiation"
    annotation (Placement(transformation(extent={{-240,-276},{-200,-236}}),iconTransformation(extent={{-240,-180},{-200,-140}})));
  //--------------------------------------------------------------
  // Direct normal radiation
  Modelica.Blocks.Interfaces.RealInput HDirNor_in(final quantity="RadiantEnergyFluenceRate",
    final unit="W/m2") if (HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HDifHor or HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HGloHor)
    "Input direct normal radiation"
    annotation (Placement(transformation(extent={{-240,-320},{-200,-280}}),iconTransformation(extent={{-240,-260},{-200,-220}})));
  //--------------------------------------------------------------
  parameter String filNamTMY3=""
    "Name of weather data file (TMY3 data)"
    annotation (Dialog(loadSelector(filter="Weather files (*.mos)", caption="Select weather file")));
  replaceable parameter BuildingSystems.Climate.WeatherData.WeatherDataFile filNamNetCDF
    "Name of weather data file (NetCDF data)"
  ...
  BuildingSystems.BoundaryConditions.WeatherData.Bus weaBus
    "Weather data bus"
    annotation (Placement(transformation(extent={{290,-10},{310,10}}),iconTransformation(extent={{190,-10},{210,10}})));
  parameter BuildingSystems.BoundaryConditions.Types.SkyTemperatureCalculation calTSky=BuildingSystems.BoundaryConditions.Types.SkyTemperatureCalculation.TemperaturesAndSkyCover
    "Computation of black-body sky temperature"
    annotation (choicesAllMatching=true,Evaluate=true,Dialog(group="Sky temperature"));
  constant Real epsCos = 1e-6
    "Small value to avoid division by 0";
equation
  connect(TDewPoi_in, weaDatTMY3.TDewPoi_in) annotation (Line(points={{-220,200},
        {-155,200},{-155,209.6},{-87.9,209.6}}, color={0,0,127}));
  connect(TDryBul_in, weaDatTMY3.TDryBul_in) annotation (Line(points={{-220,160},
        {-156,160},{-156,192},{-87.9,192}}, color={0,0,127}));
  connect(TBlaSky_in, weaDatTMY3.TBlaSky_in) annotation (Line(points={{-220,120},
        {-156,120},{-156,176},{-87.9,176}}, color={0,0,127}));
  connect(relHum_in, weaDatTMY3.relHum_in) annotation (Line(points={{-220,80},{-155,
        80},{-155,160},{-87.9,160}}, color={0,0,127}));
  connect(opaSkyCov_in, weaDatTMY3.opaSkyCov_in) annotation (Line(points={{-220,
        40},{-156,40},{-156,140.8},{-87.9,140.8}}, color={0,0,127}));
  connect(ceiHei_in, weaDatTMY3.ceiHei_in) annotation (Line(points={{-220,0},{-156,
        0},{-156,122.4},{-88.69,122.4}}, color={0,0,127}));
  connect(totSkyCov_in, weaDatTMY3.totSkyCov_in) annotation (Line(points={{-220,
        -40},{-156,-40},{-156,104.8},{-87.9,104.8}}, color={0,0,127}));
  connect(weaDatTMY3.winSpe_in, winSpe_in) annotation (Line(points={{-87.9,88.8},
        {-156,88.8},{-156,-80},{-220,-80}}, color={0,0,127}));
  connect(winDir_in, weaDatTMY3.winDir_in) annotation (Line(points={{-220,-120},
        {-220,-120},{-156,-120},{-156,72},{-87.9,72}}, color={0,0,127}));

  connect(weaDatNetCDF.longitudeDeg,weaBus.longitudeDeg);
  connect(weaDatNetCDF.longitudeDeg0,weaBus.longitudeDeg0);
  connect(weaDatNetCDF.latitudeDeg,weaBus.latitudeDeg);
  connect(weaDatNetCDF.IrrTotHor, weaBus.HGloHor) annotation (Line(points={{88,-120},
        {88,-120},{192,-120},{192,0},{300,0}}, color={0,0,127}), Text(string="%second",index=1,extent={{6,3},{6,3}}));
  connect(weaDatNetCDF.y[1],weaBus.HDirHor);
  connect(weaDatNetCDF.y[2], weaBus.HDifHor) annotation (Line(points={{88,-120},
        {184,-120},{184,0},{246,0},{246,0},{300,0}}, color={0,0,127}), Text(string="%second",index=1,extent={{6,3},{6,3}}));
  connect(weaDatNetCDF.y[3], weaBus.TDryBul) annotation (Line(points={{88,-120},
        {130,-120},{174,-120},{174,0},{300,0}}, color={0,0,127}), Text(string="%second",index=1,extent={{6,3},{6,3}}));
  connect(weaDatNetCDF.y[4], weaBus.winSpe) annotation (Line(points={{88,-120},{
        126,-120},{164,-120},{164,0},{300,0}}, color={0,0,127}), Text(string="%second",index=1,extent={{6,3},{6,3}}));
  connect(weaDatNetCDF.y[5], weaBus.winDir) annotation (Line(points={{88,-120},{
        120,-120},{154,-120},{154,0},{300,0}}, color={0,0,127}), Text(string="%second",index=1,extent={{6,3},{6,3}}));
  connect(weaDatNetCDF.y[6], weaBus.relHum) annotation (Line(points={{88,-120},{
        116,-120},{144,-120},{144,0},{300,0}}, color={0,0,127}), Text(string="%second",index=1,extent={{6,3},{6,3}}));
  connect(weaDatNetCDF.y[7], weaBus.nTot) annotation (Line(points={{88,-120},{
        134,-120},{134,0},{300,0}}, color={0,0,127}), Text(string="%second",index=1,extent={{6,3},{6,3}}));
  connect(weaDatTMY3.weaBus, weaBus) annotation (Line(points={{78,120},{146,120},{216,120},{216,0},{300,0}},color={255,204,51},thickness=0.5), Text(string="%second",index=1,extent={{6,3},{6,3}}));
      ...
      annotation (
        ...
        Icon(coordinateSystem(
            ...
            Text(
              ...
              lineColor={0,0,255}),
            Text(
              visible=(pAtmSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input),
              extent={{-190,216},{-164,184}},
              lineColor={0,0,127},
              textString="p"),
            Text(
              visible=(TDryBulSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input),
              extent={{-194,162},{-118,118}},
              lineColor={0,0,127},
              textString="TDryBul"),
            Text(
              visible=(relHumSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input),
              extent={{-190,92},{-104,66}},
              lineColor={0,0,127},
              textString="relHum"),
            Text(
            visible=(winSpeSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input),
              extent={{-196,44},{-110,2}},
              lineColor={0,0,127},
              textString="winSpe"),
            Text(
              visible=(winDirSou == BuildingSystems.BoundaryConditions.Types.DataSource.Input),
              extent={{-192,-18},{-106,-60}},
              lineColor={0,0,127},
              textString="winDir"),
            Text(
            visible=(HSou ==  BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HGloHor_HDifHor or HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HGloHor),
            extent={{-202,-88},{-112,-108}},
              lineColor={0,0,127},
              textString="HGloHor"),
            Text(visible=(HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HGloHor_HDifHor or HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HDifHor),
            extent={{-202,-142},{-116,-164}},
              lineColor={0,0,127},
              textString="HDifHor"),
            Text(
            visible=(HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HGloHor or HSou == BuildingSystems.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HDifHor),
    ...
    <li>
    Add the file to <code>BuildingSystems/Resources/weatherdata</code> (or to any directory
    for which you have write permission).
    </li>
    <li>
    On a console window, type<pre>
      cd BuildingSystems/Resources/weatherdata
    ...
    by the model
    <a href=\"modelica://BuildingSystems.BoundaryConditions.WeatherData.ReaderTMY3\">
    BuildingSystems.BoundaryConditions.WeatherData.ReaderTMY3</a>.
    </li>
    </ol>
    <!-- ============================================== -->
    ...
    the enumeration
    <a href=\"modelica://BuildingSystems.BoundaryConditions.Types.DataSource\">
    BuildingSystems.BoundaryConditions.Types.DataSource</a>
    is used as follows:
    </p>
    <table summary=\"summary\" border=\"1\" cellspacing=\"0\" cellpadding=\"2\" style=\"border-collapse:collapse;\">
    <!-- ============================================== -->
    ...
    <a href=\"modelica://BuildingSystems.BoundaryConditions.Types.RadiationDataSource\">
    BuildingSystems.BoundaryConditions.Types.RadiationDataSource</a>,
    ...
    the weather data files in the BuildingSystems library do not use the data entry from
    ...
    <p align=\"center\">
    <img alt=\"image\" src=\"modelica://BuildingSystems/Resources/Images/BoundaryConditions/WeatherData/RadiationTimeShift.png\"
    border=\"1\" />
    </p>
    <h4>References</h4>
    <ul>
    ...
    <a href=\"modelica://BuildingSystems.BoundaryConditions.SolarIrradiation.BaseClasses.Examples.SkyClearness\">
    BuildingSystems.BoundaryConditions.SolarIrradiation.BaseClasses.Examples.SkyClearness</a>
    ...
    <code>file://</code>, <code>modelica://</code> and <code>modelica://BuildingSystems</code>
    ...
    as long as the <code>BuildingSystems</code> library
